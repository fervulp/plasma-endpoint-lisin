...
def data_dir() -> Path:
    ...
    # ITS OWN DIRECTORY. This is the app manager, not the endpoint agent: it
    # holds what each flatpak application is allowed to do, which has nothing to
    # do with the endpoint's inventory and must not share a database lock with it.
    base = os.environ.get("LISIN_APPS_DATA_DIR") or os.path.expanduser(
        "~/.local/share/lisin-apps"
    )
    p = Path(base)
